...
def get_quality(cell):
    ...
    return str(int(round(float(quality[0]) / float(quality[1]) * 100))).rjust(
        3) + " %"
...
def get_signal_level(cell):
    ...
    # hacking needed...
    return matching_line(cell, "Quality=").split('Signal level=')[-1].split()[0]
...
def print_table(table):
    widths = map(max,
                 map(lambda l: map(len, l), zip(*table)))  # functional magic
...
def scan(interfaces):
    ...
    for interface in interfaces:
        ...
        if proc.returncode:
            raise OSError('{} {}'.format(_, out))
        if 'No scan results' in out:
            continue
        ...
        for cell in cells:
            ...
            cell_mac = parsed_cell['Address']
            if cell_mac not in seen_bssids:
                seen_bssids.add(cell_mac)
...
def main():
    ...
    parsed_cells = scan(DEFAULT_INTERFACES)
